...
function identityLabel(identity: HookIdentity): string {
...
/**
 * Immutable, session-scoped snapshot of all validated authorized hooks.
...
export class HookCatalog {
  ...
  static open(locations: CatalogStorageLocations): CatalogResult {
    ...
    return {
      ok: true,
      catalog: new HookCatalog(locations, loaded.snapshot),
    };
  }
  ...
  private editLocalPreset(
    ...
    if (!validateHookEntry(replacement)) {
