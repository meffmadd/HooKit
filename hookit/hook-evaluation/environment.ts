...
function logEnvironment(env: Record<string, string>, event: Event): void {
  if (process.env.PIHOOKIT_LOG_ENV?.toLowerCase() !== "true") return;

  try {
    const directory = join(dirname(globalFilePath()), ".hook-env-log");
    ...
    // Diagnostic logging must never change a hook decision.
  }
}
