...
import type { CatalogEntry } from "../hook-catalog/index.js";
import {
  SectionNavigator,
  filterPrintable,
  layoutSectionedBody,
...
/** An ordered list of hooks sharing a `source` (a display section). */
...
/**
 * Group Hooks by `source` (local first, then repositories alpha).
 ...
 */
export function groupHooksBySource(entries: readonly CatalogEntry[]): Group[] {
  ...
  return sortSources([...bySource.keys()]).map(
    (source) => ({ source, hooks: bySource.get(source)! }),
  );
}
...
export abstract class SectionedPanel {
  ...
  /**
   * Toggle the focused row's state — enable/disable in `/hooks`, membership
  ...
  protected isEmpty(): boolean {
    return this.groups.every((g) => g.hooks.length === 0);
  }
   ...
   * `/hooks` panel's confirm-remove).  Return `null` when the mode is
  ...
  /** Optional warning/note lines before focused-row hook metadata. */
  protected detailPrefixFor(_a: CatalogEntry, _width: number): string[] {
    return [];
  }

  /** Optional contextual lines after focused-row hook metadata. */
  ...
  protected detailBlockFor(a: CatalogEntry | undefined, width: number): string[] {
    ...
    return [
      ...
      ...renderHookDetail(this.theme, width, a),
   ...
   * panel-specific jump keys (e.g. `/hooks` adds `p` for the Presets section).
   */
   ...
   * bounded / unbounded) can forget its full-width muted hooks.
   */
  render(width: number, terminalHeight?: number): string[] {
    ...
    if (terminalHeight !== undefined && rendered.length > terminalHeight) {
      // Preserve both full-width hooks and the hint instead of slicing the
  ...
  protected bodyLines(
    ...
    // Header and footer (1 blank + top hook + hint line(s) + bottom hook)
    // are reserved. The active section is the anchor; adjacent section
    // headers are always shown.
    // Detail lines for the selected hook are rendered inline directly
    // below the highlighted hook row, so the active section's line budget
    // includes both hook rows and the selected row's details.  The
    ...
    const activeLen = activeGroup.hooks.length;
    const selectedHook = activeGroup.hooks[this.nav.focusedIndex];
    const detailBlock = this.detailBlockFor(selectedHook, width);
  ...
  protected exitSearch(): void {
    const kept = this.groups[this.nav.focusedSection]?.hooks[this.nav.focusedIndex];
   ...
   * query, then restore focus to the previously-highlighted hook
  ...
  protected applyFilter(): void {
    const keep = this.groups[this.nav.focusedSection]?.hooks[this.nav.focusedIndex];
    const filtered = (this.savedGroups ?? this.groups)
      .map((g) => ({
        source: g.source,
        hooks: filterSection(this.query, g.hooks),
      }))
      .filter((g) => g.hooks.length > 0);
    this.groups = filtered;
    this.nav = new SectionNavigator(filtered.map((g) => ({ items: g.hooks })));
  ...
  protected restoreFocus(a: CatalogEntry | undefined): void {
    ...
    for (let s = 0; s < this.groups.length; s++) {
      const idx = this.groups[s]!.hooks.indexOf(a); // identity (===)
      if (idx >= 0) {
        this.nav.focus = s;
