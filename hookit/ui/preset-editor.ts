/**
 * Preset editor's hook picker — a sectioned, searchable panel that *is* the
 * `/hooks` view (sections by source, fzf-style search, Tab/Shift+Tab
 ...
 * `/hooks` panel uses. This class supplies only the panel-specific hooks:
...
export class PresetEditorPanel extends SectionedPanel {
  ...
  constructor(
    ...
    this.groups = groupHooksBySource(this.hooks);
  ...
  get value(): string[] {
    ...
    // Newly selected visible hooks append in picker order. Existing refs
    // retain their exact order, including dangling and nested-preset refs.
    const added = this.hooks
      .map((a) => this.refOf(a))
  ...
  protected emptyBodyLines(_width: number): string[] {
    return [
      ...
      this.theme.fg("warning", "  No Hooks to select"),
    ];
  }
  ...
  protected renderSection(
    ...
    const maxLabelWidth = Math.max(
      0,
      ...group.hooks.map((a) => visibleWidth(a.name) + CHECK_W),
    );

    if (!focused) {
      // Dimmed static listing (no `> ` prefix, no detail block) — mirrors
      // `HooksPanel.renderSection`'s inactive branch so inactive sections
   ...
   * to the label column. Mirrors `HooksPanel.renderLabel` so search
  ...
  handleInput(data: string): PanelResult {
    // Search + navigation are shared with the `/hooks` panel (single-sourced
    ...
    // goes back (vs. `/hooks`, where `Esc` cancels).
  ...
  /** Toggle the focused hook's membership in the selection. */
...
export async function runPresetEditor(
  ...
  const hooks = state.entries.filter(
    (entry) => !isCatalogPreset(entry),
  );
