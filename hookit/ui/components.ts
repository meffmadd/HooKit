...
/**
 * Detail-block source for {@link renderHookDetail}: a Hook or Preset.
 * Fields are optional so normalized catalog and external repository entries
 * are both assignable; an omitted Hook shell renders as canonical true.
 */
export interface HookDetailEntry {
  shell?: string;
  when?: string;
  action?: Action;
...
/**
 * Render the `shell:` / `when:` / `action:` (or `hooks:`) detail block used
 * by both the /hooks panel and the install wizard's hook-entry picker.
 * `when` is only shown when present, matching the standard hook view.
 *
 * A Preset renders `hooks:` with its refs comma-joined. Every Hook
 * renders its effective shell first, then its precondition and owned Action.
 */
export function renderHookDetail(
  theme: Theme,
  width: number,
  entry: HookDetailEntry,
  /**
  ...
  if (entry.preset !== undefined) {
    detailLines(entry.preset.join(", "), "hooks: ");
...
export const HINT_ENTER_LOCK_SEARCH: [string, string] = ["Enter", "lock search"];
...
export const HINT_ESC_CLEAR_SEARCH: [string, string] = ["Esc", "clear search"];
export const HINT_D_RESET_DEFAULTS: [string, string] = ["d", "reset defaults"];
export const HINT_I_INSTALL_HOOKS: [string, string] = ["i", "install hooks"];
...
// ---------------------------------------------------------------------------
// OverlayBox — the single owner of the HooKit overlay background.
//
...
// HooKit overlay reads as a consistent surface regardless of theme.
//
// Every overlay (the /hooks panel and every install-flow dialog) goes
...
// /hooks panel passes (2, 1) to keep its inset.
// ---------------------------------------------------------------------------
...
/** Shared overlay options for every HooKit overlay (dialogs and the /hooks panel). */
...
// picker, hook-file picker, hook-entry picker) goes through here so they
...
// under the highlighted row (the hook-entry picker does this).  Omit it
// for plain pickers (repo / hook-file) which just get the shared list look
...
export async function selectDialog<T>(
  ...
  opts: {
    ...
    hint?: [string, string][];
    maxVisible?: number;
    /** Resolve the shell/when (or preset `hooks:`) preview for a given item value. */
    detailFor?: (value: string) => HookDetailEntry | undefined;
    ...
    confirmOnSelect?: {
      ...
      /** Confirm dialog title (default "Remove hook"). */
      title?: string;
  ...
  return ctx.ui.custom<SelectDialogResult<T>>((tui, theme, kb, done) => {
    ...
    const confirmShell = hasConfirm
      ? dialogShell(theme, {
          title: opts.confirmOnSelect?.title ?? "Remove hook",
          body: {
            render: () => {
...
// ---------------------------------------------------------------------------
// renderDetailList — the shared core used by BOTH the /hooks panel's
// active section and the install wizard's hook-entry picker.  It renders a
// windowed slice of items, highlights the selected row with a "> " prefix,
// and inserts the `shell:` / `when:` detail block (via renderHookDetail)
...
export interface DetailListOptions<T> {
   ...
   * `maxVisible` items centered on the selection is computed.  The /hooks
  ...
  /** Resolve the shell/when (or preset `hooks:`) preview for an item. `undefined` skips the detail. */
  detailFor: (item: T) => HookDetailEntry | undefined;
  /**
   * Optional lines rendered above the shell/when detail block for the
   * focused row (e.g. the `/hooks` panel's "removed from source repo"
   * warning for orphaned hooks).  Returns `[]` for no prefix.
   */
  detailPrefix?: (item: T) => string[];
  /**
   * Optional final lines for the focused row, emitted after warning prefixes
   * and shell/when/hooks metadata. Sectioned panels use this for contextual
...
export function renderDetailList<T>(
  ...
  for (let i = start; i < end; i++) {
    ...
    if (selected) {
      ...
      if (entry) lines.push(...renderHookDetail(theme, width, entry, opts.highlightQuery));
...
// hook-entry picker (selectDialogWithDetail).  The /hooks panel calls
...
export class DetailList<T = SelectItem> implements Component {
  ...
  constructor(
    ...
    private detailFor: (item: T) => HookDetailEntry | undefined,
    private keybindings?: KeybindingsManager,
...
// Shared by `textInputDialog` and the `/hooks` panel search mode so paste
...
// "sections" of items with arrow keys.  Used by the HooksPanel to let
...
// sectioned panel (the `/hooks` panel and the preset editor's hook
