...
async function resolveRepo(
  ...
  if (!result.ok) {
    ctx.ui.notify(`hookit: ${formatCatalogFailure(result)}`, "error");
    return null;
  }
  return newRepo;
}
...
// ---------------------------------------------------------------------------
// Step 2: pick a hook file from the repo
// ---------------------------------------------------------------------------

/** Show a picker over the fetched hook files. Returns the chosen file or null. */
async function promptHookFile(
  ctx: ExtensionContext,
  repo: string,
  files: HookFile[],
...
/** Fetch the repo's hook files and prompt the user to pick one. */
async function fetchAndPromptFile(
  ...
  let files: HookFile[];
  try {
    files = await fetchHookFiles(repo);
  } catch (err) {
    ctx.ui.notify(
      `hookit: failed to fetch hook files — ${String(err)}`,
      "error",
    );
    return null;
  }
  ...
  if (files.length === 0) {
    ctx.ui.notify(`No hook files found in ${repo}.`, "info");
    return null;
  }

  return promptHookFile(ctx, repo, files);
}

// ---------------------------------------------------------------------------
// Step 3: pick a hook entry from a hook file
// ---------------------------------------------------------------------------

/**
 * Entry picker for a single hook file.  Each entry is classified against the
 ...
 */
async function promptHookEntry(
  ctx: ExtensionContext,
  file: HookFile,
  entries: HookEntries,
  ...
  const fileName = file.path.replace(/^hooks\//, "").replace(/\.json$/, "");
  ...
  return selectDialog<string>(ctx, {
    ...
    confirmOnSelect: {
      ...
      message: (item) => {
        const e = entries[item.value];
        const kind = e && "preset" in e ? "preset" : "hook";
...
/** Fetch a hook file's entries (null on error/empty). Split out so the wizard re-fetches only when the file changes. */
async function fetchEntries(
  ctx: ExtensionContext,
  repo: string,
  file: HookFile,
): Promise<HookEntries | null> {
  let entries: HookEntries;
  try {
    entries = await fetchHookFile(repo, file.path);
  } catch (err) {
    ctx.ui.notify(
      `hookit: failed to load hook file — ${String(err)}`,
      "error",
    );
    return null;
  }
  ...
  if (Object.keys(entries).length === 0) {
    ctx.ui.notify("No valid hooks in this file.", "info");
    return null;
  }
...
async function preparePresetMembers(
  ...
  for (const identity of members.values()) {
    ...
    try {
      ...
      if (!entry) {
        ctx.ui.notify(
          `hookit: member "${identity.name}" not found in ${identity.source}.`,
          "warning",
        );
        continue;
      }
      ...
      ctx.ui.notify(
        `hookit: failed to fetch member "${identity.name}" — ${String(error)}`,
        "error",
      );
    }
  }
...
export async function installRepositoryEntry(
  ...
  if (!result.ok) {
    ctx.ui.notify(
      `hookit: failed to install "${name}" — ${formatCatalogFailure(result)}`,
      "error",
    );
    return;
  }
  ...
  for (const member of members) {
    ctx.ui.notify(
      `hookit: installed member "${member.identity.name}" (via preset).`,
      "info",
    );
  }
  ctx.ui.notify(
    `hookit: installed "${name}". Use /hooks to enable it.`,
    "info",
  );
}
...
function removeEntry(
  ...
  state.persist();
  state.updateStatus(ctx);
  ctx.ui.notify(`hookit: removed "${name}".`, "info");
}
...
async function updateEntry(
  ...
  if (!updated.ok) {
    ctx.ui.notify(
      `hookit: failed to update "${name}" — ${formatCatalogFailure(updated)}`,
      "error",
    );
    return;
  }
  ...
  if (members.length > 0) {
    ...
    if (!installedMembers.ok) {
      ctx.ui.notify(
        `hookit: updated "${name}" but failed to install preset members — ${formatCatalogFailure(installedMembers)}`,
        "error",
      );
      return;
    }
  ...
  state.updateStatus(ctx);
  ctx.ui.notify(`hookit: updated "${name}".`, "info");
}
...
export async function runInstallWizard(
  ...
  // Step 2: pick a hook file.  Loop over files until the user escapes.
  let file: HookFile | null = await fetchAndPromptFile(ctx, repo);
  ...
  while (file) {
    ...
    for (;;) {
      ...
      const result = await promptHookEntry(
        ctx,
        file,
        entries,
        installedMap,
      ...
      // `promptHookEntry` already gated the uninstall confirm for the
