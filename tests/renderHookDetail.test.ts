/**
 * Tests for the shared renderHookDetail helper used by both the
 * /hooks panel and the install wizard's hook-entry picker.
 */
...
describe("width contract", () => {
  it("keeps detail and arbitrarily wrapped hint lines within narrow widths", () => {
    const width = 12;
    const lines = [
      ...renderHookDetail(mockTheme(), width, {
...
describe("renderHookDetail — preset branch", () => {
  it("renders the hooks: label with comma-joined refs", () => {
    const lines = renderHookDetail(mockTheme(), 80, {
      preset: ["local/foo", "owner/repo/bar"],
    });
    assert.ok(
      lines.some((l) => l.includes("hooks:") && l.includes("local/foo") && l.includes("owner/repo/bar")),
      "shows the hooks: label with both refs",
    );
  });
  ...
  it("renders an empty preset array as hooks: (with nothing after)", () => {
    const lines = renderHookDetail(mockTheme(), 80, { preset: [] });
    assert.ok(
      lines.some((l) => l.includes("hooks:")),
      "shows the hooks: label even for an empty preset",
    );
  });

  it("wraps a long ref list across continuation lines", () => {
    const refs = Array.from({ length: 8 }, (_, i) => `local/ref-${i}`);
    const lines = renderHookDetail(mockTheme(), 40, { preset: refs });
    assert.ok(
      lines.some((l) => l.includes("hooks:") && l.includes("ref-0")),
      "first line shows the hooks: label and the first ref",
    );
    assert.ok(
      lines.some((l) => !l.includes("hooks:") && l.includes("ref-7")),
      "wrapped continuation line appears",
    );
  });
});

describe("renderHookDetail — owned Action", () => {
...
describe("renderHookDetail", () => {
  it("renders the shell label and command", () => {
    const lines = renderHookDetail(mockTheme(), 80, { shell: "echo hello" });
    assert.ok(
  ...
  it("wraps long shell commands onto continuation lines", () => {
    ...
    const lines = renderHookDetail(mockTheme(), 40, { shell: longShell });
    assert.ok(
  ...
  it("indents continuation lines under the label", () => {
    const lines = renderHookDetail(mockTheme(), 40, {
...
//
// renderDetailList is the pure core used by BOTH the /hooks panel's active
...
describe("DetailList (stateful component)", () => {
  ...
  function makeItems(): SelectItem[] {
    return [
      { value: "alpha", label: "alpha", description: "Alpha Hook." },
      { value: "beta", label: "beta", description: "Beta Hook." },
      { value: "gamma", label: "gamma", description: "Gamma Hook." },
    ];
  }
  ...
  it("renders the detail block directly under the highlighted row", () => {
    ...
    const rowIdx = lines.findIndex((l) => l.includes("Alpha Hook."));
    assert.ok(rowIdx >= 0, "alpha row exists");
  ...
  it("with detailFor returning undefined renders no detail and uses '> ' prefix (repo/Hook file pickers)", () => {
    ...
    const list = new DetailList(
      ...
      () => undefined, // no detail — matches the plain repo/Hook file pickers
    );
    const lines = list.render(80);
